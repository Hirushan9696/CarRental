...
import { Form, Button, Col, Row, InputGroup, FormControl, Alert, Card } from "react-bootstrap";
...
const UpdateBooking = () => {
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        navigate("/customerDashboard", { state: { role: role, userId: userId, token: token } });
      }
    } catch (error) {
  ...
  return (
    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: "100vh", backgroundColor: "#f8f9fa" }}>
      <Card className="shadow-lg p-2" style={{ width: "100%", maxWidth: "500px" }}>
        <Card.Body>
          <h3 className="text-center mb-4" style={{ color: "#222" }}>Update Booking</h3>
  ...
  <Form.Control as="select" name="pickupLocation" required value={pickupLocation} onChange={(e) => setPickupLocation(e.target.value)}>
...
<Form.Group controlId="formDestinationLocation" className="mb-3">
  ...
  <Form.Control as="select" name="destinationLocation" required value={destinationLocation} onChange={(e) => setDestinationLocation(e.target.value)}>
    {Object.keys(districtLocations).map((district) => (
      <option key={district} value={district}>{district}</option>
    ))}
  </Form.Control>
    ...
    {vehicleTypes.map((type) => (
      <option key={type} value={type}>{type}</option>
            ...
            <Form.Group className="text-center">
              <Button style={{ backgroundColor: "#222" }} type="submit">Update Booking</Button>
            </Form.Group>
          </Form>
        </Card.Body>
      </Card>
